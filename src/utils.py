...
logger_clean_amount = setup_logging("clean_amount")
logger_cart_agg_for_main = setup_logging("cart_agg_for_main")
logger_settings_for_api = setup_logging("settings_for_api")
logger_get_currency_and_stocks = setup_logging("get_currency_and_stocks")
...
def read_json_file(path_to_file_json: str) -> dict[str, Any]:
    ...
    if not path_to_file_json:
        ...
        return {}
    try:
        logger_read_json.info("Открытие файла json")
...
# Заготовка на случай необходимого возврата json формата
...
def read_excel_to_df(path_to_file_excel: str) -> list[dict[Any, Any]] | DataFrame:
    ...
    logger_excel_to_df.info("Функция возвращает dataframe с обработкой пустых ячеек")
    return df.fillna("")
...
def clean_amount(value: float | str) -> float:
    ...
    logger_clean_amount.info("Начало работы функции")
    try:
        cleaned = str(value).replace(",", ".").strip()
        return float(cleaned)
    except (ValueError, TypeError) as e:
        logger_clean_amount.error(f"Возникло исключение {e}, функция вернула результат 0.0")
        return 0.0


def cart_agg_for_main(
    operations_df: pd.DataFrame, start_data: str, stop_data: str
) -> dict[str, str | list[dict[str, float | Any]] | list[dict[str, float | Any]]]:
    ...
    logger_cart_agg_for_main.info("Начало работы функции")
    # 1. Работаем с копией и убираем пустые номера карт через .notnull()
    df = operations_df.copy()
    df = df[df["Номер карты"].notnull()]
    logger_cart_agg_for_main.info("Удаление пустых номеров карт в столбце 'Номер карты' DataFrame")
    ...
    logger_cart_agg_for_main.info("Убираем пробелы и пустые строки в номерах карт в столбце 'Номер карты'")
    ...
    logger_cart_agg_for_main.info("Убираем пробелы и меняем ',' на '.' в столбце 'Сумма операции с округлением'")
    df["Дата для фильтра"] = df["Дата операции"].map(transformed_date)
    logger_cart_agg_for_main.info("Преобразуем столбец 'Дата операции' в другой формат и создаем новый столбец"
                                  "'Дата для фильтра'")

    # 3. Фильтрация по датам
    filtered_df = df[(df["Дата для фильтра"] >= start_data) & (df["Дата для фильтра"] <= stop_data)]
    logger_cart_agg_for_main.info("Фильтруем DataFrame по заданному диапазону дат через столбец 'Дата для фильтра'")
    ...
    for card_number, group in filtered_df.groupby("Номер карты"):
        ...
        logger_cart_agg_for_main.info("Группируем отфильтрованный DataFrame по столбцу 'Номер карты'"
                                      " в каждой группе считаем сумму 'Сумма операции с округлением' ")
        ...
        logger_cart_agg_for_main.info("Добавляем в новй список словари состоящие из 'Номер карты' и "
                                      "рассчитанных значений сумм операции с округлением и кэшбэка для карты")
    ...
    logger_cart_agg_for_main.info("Сортируем отфильтрованный DataFrame по 'Сумма операции с округлением' по убыванию"
                                  "и берем первые пять строк DataFrame")
    top_transactions = []
    for index, row in top_5_raw.iterrows():
        logger_cart_agg_for_main.info(" Из отсортированного  DataFrame по 'Сумма операции с округлением' "
                                      "делаем перебор по строкам и формируем словарь с 'Дата операции', "
                                      "'Сумма операции', 'Категория', 'Описание' и записываем в новый список .")
    ...
    logger_cart_agg_for_main.info("Возвращаем ответ из новых списков и с приветствием сформированным"
                                  " внешней функцией time_of_day().")
...
def settings_for_api(path_to_settings_json: str) -> str:
    ...
    logger_settings_for_api.info("Начало работы функции")
    if not path_to_settings_json:
        logger_settings_for_api.error("Не указан путь к файлу")
        return ""
    result_from_json = read_json_file(path_to_settings_json)
    logger_settings_for_api.info("Получаем JSON файл с данными для запроса по API")
    # Если файл не прочитался или пустой — возвращаем пустую строку
    if not result_from_json:
        logger_settings_for_api.error("Не получили данные для запроса по API из внешнего файла")
        return ""
    ...
    logger_settings_for_api.info("Извлекаем данные из файла для запроса по API из внешнего файла в два списка"
                                 " по валюте и котировкам акций")

    result_currencies = [f"{currency}/RUB" for currency in currencies]
    logger_settings_for_api.info("Преобразуем список валют в нужный формат для запроса по API")

    # Складываем списки и объединяем в строку
    result_str_symbols = ",".join(stocks + result_currencies)
    logger_settings_for_api.info("Возвращаем объединенные списки валют и котировки акций единым списком")
...
def get_currency_and_stocks() -> tuple[bool, dict[str, Any]]:
    ...
    logger_get_currency_and_stocks.info("Начало работы функции")
    ...
    logger_get_currency_and_stocks.info("Получаем с помощью внешней функции нужные для запроса данные валюты"
                                        " и котировок акций")
    ...
    logger_get_currency_and_stocks.info("Производим API запрос данных")

    try:
        response = requests.get(url, timeout=5)
        data = response.json()
        status = response.status_code
        logger_get_currency_and_stocks.info("Получаем данные и распаковываем их на статус и запрос")
        if status != 200:
            print(f"Ошибка API: {data.get('message')}")
            logger_get_currency_and_stocks.error(f"Возникла ошибка API запроса {data.get('message')} "
                                                 f"возвращаем пустой словарь")
            return False, {}
        logger_get_currency_and_stocks.info("Возвращаем данные API запроса")
        return True, data
    except Exception as fault:
        print(f"Ошибка запроса: {fault}")
        logger_get_currency_and_stocks.error(f"Возникла ошибка обработки данных запроса {fault} "
                                             f"возвращаем пустой словарь")
        return False, {}
