...
def get_range_data_for_reports(target_str_date: str = None) -> datetime:
    ...
    logger_data_for_reports.info("Начало создания диапазона дат в три месяца")
    ...
    if isinstance(target_str_date, str):
        ...
        try:
            ...
            logger_data_for_reports.error(
                "Не правильно введена заданная дата, по этому берем текущую "
                "дату создания диапазона дат в три месяца"
            )
    ...
    # Если это уже объект datetime — используем его как есть
        ...
        logger_data_for_reports.info("Дата уже в формате datetime")
        end_date_obj = target_str_date

    # Во всех остальных случаях (None, числа, другой тип) — берем "сейчас"
    else:
        logger_data_for_reports.info("Дата введена в виде = None, числа, другой тип, по этому берем дату 'сейчас'")
        end_date_obj = datetime.datetime.now()

    # Вычисляем дату "3 месяца назад"
    start_date_obj = end_date_obj - timedelta(days=90)

    logger_data_for_reports.info("Возвращаем результат функции")
...
def spending_by_category(transactions: pd.DataFrame, category: str, date: Optional[str] = None) -> pd.DataFrame:
    ...
    logger_spending_by_category.info("Начало работы функции")
    # Работаем с копией и чистим категорию: убираем строки со значениями равными NaN, убираем пробелы
    # в столбце Категория .notnull()
    logger_spending_by_category.info("обработка строк категории от ненужных пробелов и значений NaN")
    ...
    logger_spending_by_category.info("В строках Сумма платежа меняем запятые на точки")
    ...
    logger_spending_by_category.info("Подготовка новой колонки Дата для фильтра для правильной сортировки")
    ...
    logger_spending_by_category.info("Получаем диапазон дат поиска транзакций через внешнюю функцию")
    ...
    logger_spending_by_category.info(
        "Фильтруем датафрейм по заданным датам и категории и 'Сумма платежа' со значением меньше ноля"
    )
    filtered_df_by_category = df.loc[
        (df["Дата для фильтра"] >= start_date)
        & (df["Дата для фильтра"] <= end_date)
...
def spending_by_weekday(transactions: pd.DataFrame, date: Optional[str] = None) -> pd.DataFrame:
    ...
    logger_spending_by_weekday.info(
        "Работаем с копией и убираем строки со значениями равными NaN с .notnull(), "
        "убираем пробелы в столбце Категория"
    )
    df = transactions.copy()
    df = df[df["Категория"].notnull()]
    df["Категория"] = df["Категория"].str.strip()
    ...
    logger_spending_by_weekday.info(
        "В 'Сумма платежа' Заменяем запятую на точку и переводим в float с помощью функции"
        " astype('требуемый тип данных')"
    )
    df["Сумма платежа"] = df["Сумма платежа"].astype(str).str.replace(",", ".").astype(float)

    # Подготовка новой колонки с датой для правильной сортировки с помощью функции pd.to_datetime
    # df["Дата для фильтра"] = df["Дата операции"].map(lambda x: datetime.datetime.strptime(x, '%m.%d.%Y %H:%M:%S'))
    logger_spending_by_weekday.info("Подготовка новой колонки datetime для правильной сортировки")
    df["Дата для фильтра"] = pd.to_datetime(df["Дата операции"], format="%d.%m.%Y %H:%M:%S", errors="coerce")
    # Удаляем строки, где дата не определилась с помощью функции df.dropna()
    df = df.dropna(subset=["Дата для фильтра"])

    # Получаем диапазон дат поиска транзакций через внешнюю функцию
    logger_spending_by_weekday.info("Получаем диапазон дат поиска транзакций через внешнюю функцию")
    start_date, end_date = get_range_data_for_reports(date)

    # Фильтрация по заданным датам
    logger_spending_by_weekday.info("Фильтруем датафрейм по заданным датам и 'Сумма платежа' со значением меньше ноля")
    filtered_df_range_date = df.loc[
        (df["Дата для фильтра"] >= start_date) & (df["Дата для фильтра"] <= end_date) & (df["Сумма платежа"] <= 0)
    ]

    # Берем модуль суммы для удобства отображения среднего используя функцию abs()
    logger_spending_by_weekday.info("'Сумма платежа' приводим к модулю для правильного вывода")
    ...
    result = (
        filtered_df_range_date.groupby(["week_dey", "Категория"])["Сумма платежа"]
        .mean()
        .round(2)  # Округляем до 2 знака
        .reset_index()  # «подписи» "week_dey", "Категория" из индекса преобразуем обратно в столбцы датафрейма
        .rename(columns={"Сумма платежа": "Средние траты"})
    ).sort_values(by="week_dey")

    logger_spending_by_weekday.info("Получаем средние траты по дням недели")
    return result
